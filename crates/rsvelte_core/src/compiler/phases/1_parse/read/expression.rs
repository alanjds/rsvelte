    ...
    match stmt {
        ...
        oxc_ast::ast::Statement::TSNamespaceDeclaration(module_decl) => {
            Some(convert_ts_module_declaration_as_node(
                arena,
                module_decl.span,
                ts_namespace_body_block(&module_decl.body),
                offset,
                line_offsets,
            ))
        }
...
/// The `{ … }` block of a namespace body, or `None` for the dotted form
/// (`namespace A.B { … }`), whose body is another namespace declaration.
fn ts_namespace_body_block<'a>(
...
/// Build the typed `TSModuleDeclaration` node `remove_typescript_nodes` inspects.
///
/// The `declare` modifier is deliberately not consulted: upstream's visitor keys
/// on the body alone, so `declare module` / `declare global` are judged by the
/// same rule as a bare `namespace`.
fn convert_ts_module_declaration_as_node(
    arena: &ParseArena,
    span: oxc_span::Span,
    block: Option<&oxc_ast::ast::TSModuleBlock<'_>>,
    offset: usize,
    line_offsets: &[usize],
) -> JsNode {
    let start = offset + span.start as usize;
...
fn convert_declaration_for_program_as_node(
    ...
    match decl {
        ...
        // `export namespace N { … }` / `export declare module "x" { … }`: the
        // declaration under an export is the same node upstream's
        // `remove_typescript_nodes` reaches through `ExportNamedDeclaration`.
...
fn convert_declaration_for_program(
    ...
    match decl {
        ...
        // TypeScript module/namespace declarations have a typed arm of their own
        // in `convert_declaration_for_program_as_node`, the only caller here.
        _ => Value::Null,
    }
}
