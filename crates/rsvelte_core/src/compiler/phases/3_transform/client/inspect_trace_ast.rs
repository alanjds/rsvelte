...
impl<'src> TraceCollector<'src> {
    ...
    fn rewrite_body(
        ...
        self.edits.push((
            ...
            body.span.end,
            format!(
                "{{ return {awaited}$.trace({label_thunk}, {asyncness}() => {{ {rest} }}); }}"
            ),
        ));
    }
}
...
impl<'a, 'src> Visit<'a> for TraceCollector<'src> {
    ...
    fn visit_function(&mut self, it: &Function<'a>, flags: oxc_syntax::scope::ScopeFlags) {
        ...
        if let Some(body) = &it.body {
            ...
            self.rewrite_body(body, it.r#async, false, own, it.span.start);
            self.parent_label = None;
        }
    ...
    fn visit_arrow_function_expression(&mut self, it: &ArrowFunctionExpression<'a>) {
        let label = self.parent_label.take();
        self.parent_label = label;
        self.rewrite_body(&it.body, it.r#async, it.expression, None, it.span.start);
        self.parent_label = None;
...
mod tests {
    ...
    fn an_unlabelled_call_falls_back_to_the_function_name_and_position() {
        let out = dev("let base = 1;\nexport function go() { $inspect.trace(); return base; }").unwrap();
        assert!(
            out.contains("() => 'go (m.svelte.js:2:7)'"),
            "got: {out}"
        );
    }
    ...
    fn a_nested_function_is_traced_where_it_stands() {
        let out = dev("export function go() { function inner() { $inspect.trace(); } return inner; }")
            .unwrap();
    ...
    fn without_dev_the_statement_is_only_removed() {
        let out =
            transform_module_inspect_trace("export function go() { $inspect.trace(); return 1; }", false, false, Some("m.svelte.js"))
                .unwrap();
