...
/// Upstream rewrites `$$props` **reads** (`read:` in `3-transform/client/index.js`);
/// the first argument of a generated `$.prop` / `$.bind_prop` / `$.legacy_rest_props`
/// call is a binding position, not a read, so only that one keeps the name.
fn replace_dollar_props_reads(line: &str) -> String {
    const BINDING_POSITION: [&str; 3] = ["$.prop(", "$.bind_prop(", "$.legacy_rest_props("];
    let mut out = String::with_capacity(line.len());
    let mut last = 0;
    for m in REGEX_DOLLAR_PROPS.find_iter(line) {
        out.push_str(&line[last..m.start()]);
        if BINDING_POSITION
            .iter()
            .any(|call| line[..m.start()].ends_with(call))
        {
...
pub(crate) fn transform_client(
    ...
    if let Some(ref content) = analysis.instance_script_content {
        ...
        // In legacy mode, replace $$props reads with $$sanitized_props, mirroring
        // the official compiler's read: (node) => ({ ...node, name: '$$sanitized_props' }).
        if !analysis.runes && (analysis.uses_props || analysis.uses_rest_props) {
            ...
            for line in lines {
                result_lines.push(replace_dollar_props_reads(line));
            }
