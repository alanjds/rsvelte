...
use crate::compiler::phases::phase3_transform::shared::js_scan::{find_code_ident, skip_opaque};
...
pub(super) fn transform_client_runes_with_skip_and_state<'a>(
    ...
    if !dev && !inspect_is_store_sub {
        while let Some(pos) = find_code_ident(result.as_bytes(), b"$inspect.trace(") {
    ...
    if !dev
        ...
        && let Some(pos) = find_code_ident(result.as_bytes(), b"$inspect(")
    {
        {
