...
fn mapping_at_substring(code: &str, needle: &str, mappings: &[Mapping]) -> [i64; 4] {
    ...
    mapping_at_index(code, idx, mappings)
}

/// Mapping at an exact generated byte index. The source fixtures are ASCII, so
/// their byte index is also the source-map column used by this test suite.
fn mapping_at_index(code: &str, index: usize, mappings: &[Mapping]) -> [i64; 4] {
    let (gen_line, gen_col) = generated_line_column(code, index);
    let m = mappings
        .iter()
        .find(|m| m.gen_line as usize == gen_line && m.gen_column as usize == gen_col)
        .unwrap_or_else(|| panic!("no mapping at line {gen_line} col {gen_col}: {mappings:?}"));
    [
...
#[test]
fn identifiers_and_literals_map_both_span_boundaries() {
    let m = mapped("const value = 'text';\nvalue;");

    let binding_start = m.code.find("value").expect("binding in output");
    let binding_end = binding_start + "value".len();
    let literal_start = m.code.find("'text'").expect("literal in output");
    let literal_end = literal_start + "'text'".len();
    let reference_start = m.code.rfind("value").expect("reference in output");
    let reference_end = reference_start + "value".len();

    assert_eq!(
        mapping_at_index(&m.code, binding_start, &m.mappings)[2..],
        [0, 6]
    );
    assert_eq!(
        mapping_at_index(&m.code, binding_end, &m.mappings)[2..],
        [0, 11]
    );
    assert_eq!(
        mapping_at_index(&m.code, literal_start, &m.mappings)[2..],
        [0, 14]
    );
    assert_eq!(
        mapping_at_index(&m.code, literal_end, &m.mappings)[2..],
        [0, 20]
    );
    assert_eq!(
        mapping_at_index(&m.code, reference_start, &m.mappings)[2..],
        [1, 0]
    );
    assert_eq!(
        mapping_at_index(&m.code, reference_end, &m.mappings)[2..],
        [1, 5]
    );
}
...
fn block_braces_map_to_their_source_positions() {
    ...
    let close = m.code.find('}').expect("closing brace");

    assert_eq!(mapping_at_index(&m.code, open, &m.mappings)[2..], [0, 13]);
    assert_eq!(mapping_at_index(&m.code, close, &m.mappings)[2..], [0, 14]);
}
