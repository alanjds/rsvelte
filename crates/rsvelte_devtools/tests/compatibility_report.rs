...
#[path = "../../rsvelte_core/tests/common/mod.rs"]
mod common;
#[path = "../../rsvelte_projection/tests/common/svelte2tsx.rs"]
...
fn generate_compatibility_report() {
    ...
    // svelte2tsx belongs to the projection crate, while this cross-product
    // dashboard lives in devtools. Convert the projection fixture outcomes
    // into the shared report schema here so rsvelte_core keeps no reverse
    // dependency on its higher-level projection consumer.
    print!("Running svelte2tsx tests... ");
    if let Some(outcomes) = projection_fixtures::iter_svelte2tsx_outcomes() {
        let mut projection = CategoryResult::new("svelte2tsx");
        for outcome in outcomes {
            let status = match outcome.status {
                projection_fixtures::TestStatus::Passed => TestStatus::Passed,
                projection_fixtures::TestStatus::Failed => TestStatus::Failed,
                projection_fixtures::TestStatus::Skipped => TestStatus::Skipped,
                projection_fixtures::TestStatus::Error => TestStatus::Error,
            };
            ...
            projection.add_sample(SampleResult {
                name: outcome.name,
                status,
                error,
                skip_reason,
        ...
        println!(
            "{}/{} passed ({:.1}%)",
            projection.stats.passed,
            projection.stats.run_count(),
            projection.stats.pass_percentage()
        );
        report.add_category(projection);
    } else {
