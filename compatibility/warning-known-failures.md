...
`warning-known-failures.<target>.json` holds the same 83 entries on all four,
...
## Warning codes (`warning-known-failures.<target>.json`, 83 entries each)
...
Not every entry is equally bad. Of the 83 entries that still diverge, **22 are
...
Partition of `warning-known-failures.<target>.json` by direction: `22 + 61`

**73 of the 83 arrived with the wave-2 enrolment (#3130)**, which took the
corpus from 37 corpus sources to 104. The codes involved, counted over
entries rather than tuples (they sum to exactly 83 — every listed entry diverges
on one code): `css_unused_selector` 48, `state_referenced_locally` 22,
`non_reactive_update` 8, `a11y_consider_explicit_label` 4,
